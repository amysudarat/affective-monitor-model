...
-Unroll 100 time steps
    - Each step input size: 24 x 1
    - Total per unroll: 24 x 100
        -Feedforward Neural Network input size: 24 x 100
    - 1 Hidden layer
    -Output dimension : 4 
"""
...
class myLSTM_valence(nn.Module):
    def __init__(self,input_dim, hidden_dim, layer_dim, output_dim):
        super(myLSTM_valence,self).__init__()
        # Hidden dimensions
    ...
    def forward(self,x):
        ...
        c0 = torch.zeros(self.layer_dim, x.size(0), self.hidden_dim).requires_grad()
        ...
        # out.size() --> 100, 28, 4
        # out[:, -1, :] --> 100, 4 --> just want last time step hidden states! 
        out = self.fc(out[:, -1, :]) 
        # out.size() --> 100, 4
        return out
